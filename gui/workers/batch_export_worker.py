...
class BatchExportWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # (Security Note: Unvalidated Session IDs - session_ids come from the GUI checkbox selection.
            #  While these are generated by the application itself (not user-typed), if get_selected_session_ids
            #  were ever fed from an untrusted source, session_id values would be passed directly to
            #  extract_session_bundle() without validation. Invalid IDs produce empty results, not errors,
            #  so there is no injection vector here. This is documented for future code reviewers.)
            #
            # (Security Note: Concurrent Filesystem Writes - The worker runs in a background QThread and writes
            #  files directly to disk. If multiple BatchExportWorker instances were instantiated concurrently
            #  (e.g., user clicks export twice rapidly before the first finishes), they could write to the same
            #  dest_dir simultaneously, causing file collisions and potential data corruption. The caller should
            #  disable the export button until the worker finishes (which this code does via setEnabled(False),
            #  but the enforcement is advisory only - external callers could bypass it.)
